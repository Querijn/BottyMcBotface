...
export default class RiotAPILibraries {
    ...
    this.fetchSettings = {
      headers: {
        ...
        Authorization: `Basic ${Buffer.from(
          personalSettings.github.username +
            ":" +
            personalSettings.github.password
        ).toString("base64")}`,
  ...
  public onLibs(
    ...
    if ("name" in message.channel) {
      for (const [topic, tags] of Object.entries(CHANNEL_TOPICS)) {
  ...
  private async describeAPILibrary(
    json: GithubAPIStruct,
    tags: string[] = ["v4"]
    ...
    const repoResponsePromise = fetch(
      `https://api.github.com/repos/${libraryInfo.owner}/${libraryInfo.repo}`,
      this.fetchSettings
    );

    // Make a list of the links
    const githubLink = `github.com/${libraryInfo.owner}/${libraryInfo.repo}`;
    let links = libraryInfo.links
      ? libraryInfo.links.map((link) => `[${link.name}](${link.url})`)
      : []; // Can be empty array or null, sigh
  ...
  private async initList() {
    try {
      const response = await fetch(
        this.settings.riotApiLibraries.baseURL,
        this.fetchSettings
      );
      if (response.status !== 200) {
        console.error(
          this.settings.riotApiLibraries.githubErrorList + response.status
        );
        return;
      }

      const languageNames = ((await response.json()) as GithubAPIStruct[]).map(
        (x) => x.name
      );
      ...
      for (const language of languageNames) {
        try {
          ...
          console.warn(
            `Unable to fetch library data for language ${language}: ${e}`
          );
        }
      }

      console.log(
        "Riot API library languages updated: " + this.languageList.join(", ")
      );
    } catch (e) {
    ...
    this.timeOut = setTimeout(
      this.initList.bind(this),
      this.settings.riotApiLibraries.checkInterval
    );
  }

  private async getList(message: Discord.Message) {
    const reply = this.settings.riotApiLibraries.languageList.replace(
      "{languages}",
      "`" + this.languageList.join(", ") + "`"
    );
    message.channel.send(reply);
  }

  private async getLibrariesForLanguage(
    language: string,
    tags: string[] = ["v4"]
  ): Promise<LibraryDescription[]> {
    const response = await fetch(
      this.settings.riotApiLibraries.baseURL + language
    );
    switch (response.status) {
      ...
      default: {
        throw new Error(
          this.settings.riotApiLibraries.githubErrorLanguage + response.status
        );
      }
    }
    ...
    if (
      !Array.isArray(libraryList) ||
      libraryList.length === 0 ||
      !libraryList[0].sha
    ) {
      throw new Error(this.settings.riotApiLibraries.noLanguage + language);
    }
    const promises = libraryList.map((lib) =>
      this.describeAPILibrary(lib, tags)
    );
    const libraryDescriptions = (await Promise.all(promises)).filter(
      (l) => l.valid && l.library
    ); // Only valid ones
  ...
  private async getListForLanguage(
    message: Discord.Message,
    language: string,
    tags: string[] = ["v4"]
  ): Promise<void> {
    ...
    for (const [key, values] of Object.entries(
      this.settings.riotApiLibraries.aliases
    )) {
    ...
    const editMessagePromise = message.channel.send(
      `Fetching the list of libraries for ${language}, this post will be edited with the result.`
    );

    let libraryDescriptions: LibraryDescription[] = [];
    try {
      libraryDescriptions = (
        await this.getLibrariesForLanguage(language, tags)
      ...
      embed.addFields([
        {
          ...
          value: `${
            desc.library.description ? desc.library.description + "\n" : " "
          }${desc.links.join(", ")}`
        }
      ]);
    }
